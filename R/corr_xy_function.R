...
corr.two.data.sets.v2 <- function(
  ...
  df.results <- data.frame(matrix(nrow = 0, ncol = 10))
  ...
  for(per in y_factors){
    ...
    dfpp <- foreach(prot = x_factors, .combine = 'rbind')%dopar%{
      ...
      }, error = function(e){print(paste0("Failed for: ", per))}
      )
    }
    stopImplicitCluster()
    end_time <- Sys.time()
  ...
  if (threshold_test == "Corr"){
    colnames(df.results) <- c(y_factor_name, x_factor_name, "pval", "rval", "beta", "R_squared", "se_slope", "se_intercept",  "n_cells", "fdr")
    df.results <- subset(df.results, df.results$rval!=0)
  } else if (threshold_test == "MI"){
    colnames(df.results) <- c(y_factor_name, x_factor_name, "pval", "mi", "beta", "R_squared", "se_slope", "se_intercept",  "n_cells", "fdr")
  }
